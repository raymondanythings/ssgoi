...
Use the `scope` option to control when an element animates:

- `global` (default): animate whenever the element mounts or unmounts
- `local`: animate only while a parent transition is running
